 ...
 *
 * This daemon is derived from btbridged from
 * https://github.com/openbmc/btbridge. There is no need to queue messages for
 * SSIF since they should not arrive out of sequence.
 */
...
class SmsChannel
{
  public:
    ...
    void processMessage(const boost::system::error_code& ecRd, size_t rlen)
    {
        ...
        printf("Read req msg: NETFN=0x%02x, LUN=0x%02x, CC=0x%02x\n",
                netfn, lun, cmd);
        // copy out payload
        ...
        bus->async_method_call(
            [this, netfnCap{netfn}, lunCap{lun},
             cmdCap{cmd}](const boost::system::error_code& ec,
                ...
                printf("Send rsp msg: NETFN=0x%02x, LUN=0x%02x, CC=0x%02x\n",
                        netfn, lun, cmd);
                if (verbose)
                {
                    log<level::INFO>(
...
int main(int argc, char* argv[])
{
    ...
    std::string channel;
    app.add_option("-d,--device", channel,
                   "channel name. Default is /dev/ipmi-ssif-host");
    bool verbose = false;
    ...
    printf("STEP2\n");
    // Create the channel, listening on D-Bus and on the SSIF device
